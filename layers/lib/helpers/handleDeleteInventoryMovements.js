const handleDeleteInventoryMovements = async (req, res) => {
  ...
  if (inventoryMovementIds?.length > 0) await res.connection.query(deleteInventoryMovementsDetails(inventoryMovementIds))
  if (inventoryMovementIds?.length > 0) await res.connection.query(deleteInventoryMovements(inventoryMovementIds))
